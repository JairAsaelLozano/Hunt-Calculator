...
import { Copy, Save, Upload, Download, Users, Clock, Coins } from 'lucide-react';
...
const HuntSessionAnalyzer: React.FC = () => {
  ...
  const parseSessionData = useCallback((text: string): SessionData | null => {
    try {
      ...
      // Parse session info
      const sessionLine = lines.find(line => line.startsWith('Session data:'));
      if (!sessionLine) throw new Error('Session data line not found');
      
      const dateMatch = sessionLine.match(/From (\d{4}-\d{2}-\d{2}, \d{2}:\d{2}:\d{2}) to (\d{4}-\d{2}-\d{2}, \d{2}:\d{2}:\d{2})/);
      ...
      const lootType = (lootTypeLine?.split(':')[1]?.trim() || 'Leader') as 'Leader' | 'Market' | 'Split';
      
      const totalLootLine = lines.find(line => line.startsWith('Loot:'));
      const totalLoot = parseInt(totalLootLine?.split(':')[1]?.trim().replace(/,/g, '') || '0');
      
      const totalSuppliesLine = lines.find(line => line.startsWith('Supplies:'));
      const totalSupplies = parseInt(totalSuppliesLine?.split(':')[1]?.trim().replace(/,/g, '') || '0');
      
      const balanceLine = lines.find(line => line.startsWith('Balance:'));
      const totalBalance = parseInt(balanceLine?.split(':')[1]?.trim().replace(/,/g, '') || '0');
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line && !line.includes(':') && !line.startsWith('Session') && !line.startsWith('Loot') && !line.startsWith('Supplies') && !line.startsWith('Balance')) {
          ...
          currentPlayer.loot = parseInt(line.split(':')[1].trim().replace(/,/g, ''));
        } else if (line.includes('Supplies:') && currentPlayer.name) {
          currentPlayer.supplies = parseInt(line.split(':')[1].trim().replace(/,/g, ''));
        } else if (line.includes('Balance:') && currentPlayer.name) {
          currentPlayer.balance = parseInt(line.split(':')[1].trim().replace(/,/g, ''));
        } else if (line.includes('Damage:') && currentPlayer.name) {
          currentPlayer.damage = parseInt(line.split(':')[1].trim().replace(/,/g, ''));
        } else if (line.includes('Healing:') && currentPlayer.name) {
          currentPlayer.healing = parseInt(line.split(':')[1].trim().replace(/,/g, ''));
        }
      }
  ...
  const copyAllTransfers = () => {
    const commands = transfers.map(transfer => 
  ...
  const totalProfit = transfers.reduce((sum, transfer) => sum + transfer.amount, 0);
        ...
        <div className="bg-gray-800 rounded-lg shadow-lg">
          <div className="border-b border-gray-700 px-6 py-4">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-bold">Hunt Session Analyzer</h1>
            </div>
          </div>
              ...
              {parsedSession ? (
                <div className="space-y-6">
                  <div className="bg-gray-700 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button 
                        onClick={() => copyToClipboard(JSON.stringify(parsedSession, null, 2))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm text-gray-400">Team session</div>
                          <div className="font-medium">{parsedSession.players.length} players</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="font-medium">{parsedSession.duration}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="font-medium">{parsedSession.lootType}</div>
                        </div>
                      </div>
                    </div>

                    <div className="text-sm text-gray-400 mb-2">
                      {formatDate(parsedSession.startDate)} - {formatDate(parsedSession.endDate)}
                    </div>
                  </div>
                          ...
                          <div key={index} className="flex items-center justify-between p-3 bg-gray-800 rounded-md">
                            <div className="flex items-center gap-3">
                              <div className="text-sm">
                                <span className="text-yellow-400">{transfer.from}</span>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-medium">{formatNumber(transfer.amount)}</span>
                              <button 
                      ...
                      {totalProfit > 0 && (
                        <div className="mt-4 p-3 bg-orange-600/20 rounded-md">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium">Total session profit</span>
                            <div className="flex items-center gap-2">
                              <span className="text-orange-400 text-sm">●</span>
                              <span className="font-bold">{formatNumber(parsedSession.totalLoot - parsedSession.totalSupplies)} total</span>
                              <span className="text-xs text-gray-400">
                                ({formatNumber((parsedSession.totalLoot - parsedSession.totalSupplies) / parsedSession.players.length)} per player)
                              </span>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="bg-gray-700 rounded-lg p-4">
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {parsedSession.players.map((player, index) => {
                            const balancePerPlayer = parsedSession.totalBalance / parsedSession.players.length;
                            const shouldReceive = player.supplies + balancePerPlayer;
                            ...
                            return (
                              <tr key={index} className="border-b border-gray-600/50">
                                <td className="py-2">
                                  <div className="flex items-center gap-2">
                                    {player.isLeader && <span className="text-yellow-400 text-xs">👑</span>}
                                    <span className={player.isLeader ? 'font-medium text-yellow-400' : ''}>
                                      {player.name}
                                    </span>
                                  </div>
                                </td>
                                <td className="text-right py-2 text-green-400">{formatNumber(player.loot)}</td>
                                <td className="text-right py-2 text-red-400">{formatNumber(player.supplies)}</td>
                                <td className={`text-right py-2 ${player.balance >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                  {formatNumber(player.balance)}
                                </td>
                                <td className="text-right py-2 text-blue-400 font-semibold">
                                  {formatNumber(Math.round(shouldReceive))}
                                </td>
                                <td className="text-right py-2">{formatNumber(player.damage)}</td>
                                <td className="text-right py-2">{formatNumber(player.healing)}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                    ...
                    <div className="mt-4 p-3 bg-gray-600/30 rounded-md text-xs text-gray-300">
                      <div className="grid grid-cols-1 gap-2">
                        <div>
                          <strong>Formula:</strong> Should Receive = Individual Supplies + (Total Balance ÷ {parsedSession.players.length} players)
                        </div>
                        <div className="text-orange-300">
                          <strong>Balance per player:</strong> {formatNumber(parsedSession.totalBalance)} ÷ {parsedSession.players.length} = <span className="font-bold">{formatNumber(Math.round(parsedSession.totalBalance / parsedSession.players.length))}</span>
                        </div>
                        <div className="text-blue-300">
                          <strong>Logic:</strong> Leader transfers to each player their supplies + their fair share of the total balance
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-center h-96">
                  <div className="text-center text-gray-400">
                    ...
                    <p className="text-sm">Paste your hunt session data to get started</p>
                  </div>
                </div>
              )}
            </div>
